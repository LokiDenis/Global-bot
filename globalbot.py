...
async def addGlobal(ctx):
    if ctx.author.guild_permissions.administrator:
        if not guild_exists(ctx.guild.id):
            ...
            with open('servers.json', 'w') as f:
                json.dump(servers, f, indent=4)
            embed = discord.Embed(title="**Willkommen im GlobalChat von Angi World™**",
...
@bot.event
async def on_ready():
    activity = discord.Game(name="Hilfe » -help", type=3)
...
async def sendAll(message: Message):
    ...
    links = '[Bot einladen](https://discordapp.com/oauth2/authorize?client_id=698144121784631296&scope=bot&permissions=8) ║ '
    ...
    if len(globalchat["invite"]) > 0:
        ...
        links += f'[Support](https://discord.gg/hZjcpne)'
